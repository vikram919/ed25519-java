...
public class X25519KeyAgreement {
    ...
    public X25519KeyAgreement(PrivateKey key) throws InvalidKeyException {
        if (!(key instanceof X25519PrivateKey)) {
            throw new InvalidKeyException("X25519"
                    + " key agreement requires "
                    + EdDSAPrivateKey.class.getSimpleName() + " for initialization");
        }
    ...
    public void doFinal(PublicKey key) throws InvalidKeyException {
        if (!(key instanceof X25519PublicKey)) {
            throw new InvalidKeyException("X25519"
                    + " key agreement requires "
                    + EdDSAPublicKey.class.getSimpleName() + " for doFinal");
        }
        mSharedSecret = new byte[KEY_SIZE];
        curve(mSharedSecret, mPrivateKey, key.getEncoded());
    }
    ...
    public SecretKey generateSecret(String algorithm)
        ...
        return new X25519SecretKey(generateSecret());
    }
